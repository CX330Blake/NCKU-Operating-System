 ...
 */
static struct dentry *osfs_lookup(struct inode *dir, struct dentry *dentry,
				  unsigned int flags)
{
	...
	dir_data_block =
		sb_info->data_blocks + parent_inode->i_block * BLOCK_SIZE;
...
static int osfs_iterate(struct file *filp, struct dir_context *ctx)
{
	...
	dir_data_block =
		sb_info->data_blocks + osfs_inode->i_block * BLOCK_SIZE;
 ...
 */
static int osfs_create(struct mnt_idmap *idmap, struct inode *dir,
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	bool had_allocated_block = false;
	...
	if (parent_inode->i_blocks == 0) {
		ret = osfs_alloc_data_block(sb_info, &parent_inode->i_block);
		if (ret)
			return ret;
		parent_inode->i_blocks = 1;
		dir->i_blocks = parent_inode->i_blocks;
	}
	...
	allocated_block = osfs_inode->i_block;
	had_allocated_block = osfs_inode->i_blocks != 0;
	// init osfs_inode attribute
	osfs_inode->i_block = 0;
	osfs_inode->i_size = 0;
	osfs_inode->i_blocks = 0;
	if (allocated_block == 0 && had_allocated_block)
		goto skip_data_block_allocation;
	if (allocated_block == 0) {
		ret = osfs_alloc_data_block(sb_info, &allocated_block);
		if (ret) {
			pr_err("osfs_create: Failed to allocate data block\n");
			if (sb_info) {
				clear_bit(inode->i_ino, sb_info->inode_bitmap);
				sb_info->nr_free_inodes++;
			}
			iput(inode);
			return ret;
...
skip_data_block_allocation:
	osfs_inode->i_block = allocated_block;
	...
	osfs_inode->i_uid = i_uid_read(inode);
	osfs_inode->i_gid = i_gid_read(inode);
	osfs_inode->i_links_count = inode->i_nlink;
	...
	ret = osfs_add_dir_entry(dir, inode->i_ino, dentry->d_name.name,
				 dentry->d_name.len);

	if (ret) {
		pr_err("osfs_create: Failed to add directory entry\n");
		if (osfs_inode->i_blocks && sb_info)
	...
	parent_inode->__i_mtime = parent_inode->__i_ctime =
		current_time(dir);
